...
Static Function FolderDic(oDlg, np) 
    ...
    oP1:Align :=CONTROL_ALIGN_TOP
        ...
        @ 005, 030 GET cAliasDic of oP1 SIZE 040, 09 PIXEL VALID DicBrowse(cAliasDic, oP2, np, oIndice, oMsg)
        ...
        THButton():New(020, 550, "Colunas"    , oP1, {|| ReorgColu(cAliasDic, oP2, np, oIndice, oMsg )  }, 40, 10, oFontB, "Reorganize as colunas") 
...
Static Function ReorgColu(cAliasDic, oP2ant, np, oIndice, oMsg )
    ...
    DEFINE MSDIALOG oDlg FROM 010,010 TO 550,600 TITLE "Reorganizar colunas (visão) " + cAliasDic + " com " + Alltrim(Str(Len(aStruct))) + " campos"  PIXEL 
        ...
        oP1:Align :=CONTROL_ALIGN_TOP
            ...
            @ 007, 002 SAY "Ordem "   of oP1 SIZE 020, 10 PIXEL
            @ 005, 032 MSCOMBOBOX oOrdem    VAR cOrdem    ITEMS aOrdem   SIZE 40, 09 PIXEL OF oP1 VALID OrdemEstru(cOrdem, oLbx1, aStruct, np) 

            @ 007, 130 SAY "Busca"    of oP1 SIZE 020, 10 PIXEL
            @ 005, 150 GET cPesquisa  of oP1 SIZE 080, 09 PIXEL PICT "@!" VALID BuscaEstru(oLbx1, cPesquisa) 

            @ 025, 002 SAY "Inserir lista "   of oP1 SIZE 030, 10 PIXEL
            @ 023, 032 GET cLista             of oP1 SIZE 210, 09 PIXEL PICT "@!" VALID InsLista(cLista, oLbx1, oLbx2)  
            oBH   := THButton():New(026, 250, "Histórico"       ,oP1, {|| MenuCmp(oLbx1, oLbx2, oBH)    }, 45, 10, oFontB, "Lista com o histórico") 

        oP2:= TPanelCss():New(,,, oDlg)
        oP2:SetCoors(TRect():New(0, 0, 38, 100))
        oP2:Align :=CONTROL_ALIGN_ALLCLIENT
        
            @ 000, 000 LISTBOX oLbx1 FIELDS HEADER "Visão original" SIZE 130,200 PIXEL OF oP2 
            ...
            oP3:SetCoors(TRect():New(0, 0, 70, 70))
        ...
        oPI:SetCoors(TRect():New(0, 0, 30, 500))
        oPI:Align :=CONTROL_ALIGN_BOTTOM
            THButton():New(003, 005, "Ok"      ,oPI, {|| NewOk(oLbx1, oLbx2, cAliasDic, oP2ant, np, oIndice, oMsg), oDlg:End() }, 20, 10, oFontB, "Confirma a nova visão?") 
            THButton():New(003, 030, "Cancela" ,oPI, {|| lOk:= .F., oDlg:End() }, 30, 10, oFontB, "Fecha a tela!") 
...
Static Function MenuCmp(oLbx1, oLbx2, oOwner)
    Local nX := 0
    Local oMenu
    Local cAux    := ''
    Local bBlock  :={||}
    
    LeHst(__aHstCmp, "campo")

    oMenu := tMenu():new(0, 0, 0, 0, .T., , oOwner)
    oMenu:Add(tMenuItem():new(oMenu, "Limpar Histórico", , , , {|| If(MsgYesNo("Confirma a limpeza do histórico?"), (GrvHst(__aHstCmp, "campo", ""), .t. ), .t.) }, , , , , , , , , .T.))
...
Static Function NewOk(oLbx1, oLbx2, cAliasDic, oP2ant, np, oIndice, oMsg)
    Local aCampos:= oLbx1:aArray 
    ...
    For nx:= 1 to len(aNew)
        aadd(__aNewBrw[np] , aNew[nx, 1])
        If nx < 20
            cHist   += aNew[nx, 1]+","
            cHisOri += aCampos[nx, 1]+","
        EndIf 
    Next 
    If cHist <> cHisOri
        GrvHst(__aHstCmp, "campo", cHist)
